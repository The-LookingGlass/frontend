...
def get_job_list(wsdl, fromDate, toDate, regionID, CategoryIds):
    ...
    with client.settings(strict = False):
        ...
        data = client.service.SearchJobs(excludeAgencies = 1,
         regionIds = regionID, 
...
categories = {"Accountancy+Finance":21, 
"Banking+Insurance":126, 
"Education+Academic":[9, 269]}
...
for key, value in categories.items():  # Loop through the categories. A file will be created for each category.
    category_name = key + ".json"
    CategoryIds = value 
    mydir = r"F:\frontend\data"
    file_name = os.path.join(mydir, category_name)
